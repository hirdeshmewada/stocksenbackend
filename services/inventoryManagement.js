...
const createProduct = async (name, price, inventory = 0) => {
  try {
    ...
    await product.save();
    console.log(
      `Product ${productName} created with price ${price} and inventory ${inventory}`
    );
...
const increaseInventory = async (name, quantity) => {
  try {
    const productName = name.toLocaleLowerCase();
    console.log("increaseInventory", name, quantity, productName);
    ...
    const product = await Product.findOne({ name: "happy" });
    if (!product) {
    ...
    console.log(
      `Increased inventory for ${productName} by ${incrementValue}. New inventory: ${product.inventory}`
    );
...
const decreaseInventory = async (name, quantity) => {
  try {
    ...
    console.log(
      `Decreased inventory for ${productName} by ${decrementValue}. New inventory: ${product.inventory}`
    );
    return { success: true, inventory: product.inventory };
  } catch (error) {
